...
** HACK Autoloads
   ...
   These autoloads are needed for some reason. Can’t figure out why they aren’t
   being setup later on.
...
(load custom-file 'noerror)
    #+END_SRC
** Setup use-package
   First, let’s set some use-package global variables that are needed before
   require it.
...
(setq use-package-always-defer t
   ...
   site-paths should be available (unless you don’t have Nix), we can skip this step.
  ...
  myPackages = builtins.fromJSON (builtins.readFile (
    with customEmacsPackages.melpaPackages;
      stdenv.mkDerivation {
        ...
        unpackPhase = "true";
        installPhase = ''
cp ${./README.el} README.el
cp -r ${./lisp} lisp
cp ${site-paths} site-paths.el
export HOME=$PWD
emacs --batch \
      -L ${bind-key}/share/emacs/site-lisp/elpa/bind-key-* \
      -L ${use-package}/share/emacs/site-lisp/elpa/use-package-* \
      -L lisp -L . -l use-package-list \
      --eval "(use-package-list \"README.el\")" > $out
        '';
      }));
  #+END_SRC
  #+BEGIN_SRC nix :tangle no
  emacsWrapper = packages: runCommand "emacs-packages-deps"
   { inherit emacs; explicitRequires = packages; lndir = xorg.lndir; }
   ''
     mkdir -p $out/bin
...
emacs --batch \
      --eval "(let ((default-directory \"${emacsWrapper
        ((requiredPackages emacsPackagesNg myPackages) ++
         [emacsPackagesNg.melpaPackages.use-package])
...
#+BEGIN_SRC nix
  requiredPackages = epkgs: map (x: if x == "rtags" then pkgs.rtags
...
notifications:
  ...
  #+BEGIN_EXPORT html :tangle no
<a href="https://travis-ci.org/matthewbauer/bauer.svg">
