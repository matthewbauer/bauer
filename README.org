...
when wanting to start for the first time. If you want to enable the big version,
you can just do ‘nixpkgs.config.bauer.small = false’. Alternatively, if you
/want/ the small version, just do ‘nixpkgs.config.bauer.small = true’.
