...
really large number permanently. This works well until the garbage is actually
...
[[./site-lisp/set-defaults.el][set-defaults]] provides an easy way to override the default custom files. This
...
#+BEGIN_SRC emacs-lisp
  ...
  (set-defaults
   ...
   '(grep-find-ignored-directories '("SCCS" "RCS" "CVS" "MCVS" ".src" ".svn" ".jj" ".git" ".hg" ".bzr" "_MTN" "_darcs" "{arch}" "node_modules" "build" "dist"))
...
=set-envs= is provided by [[./site-lisp/set-defaults.el][set-defaults]]. We can use it like
...
restart Emacs for the upgrade to take place. See [[./site-lisp/installer.el][installer.el]] for documentation.
...
A better version of ‘completing-read’ that isn’t as heavy duty as company.
...
(use-package vertico
  ...
  :demand
  :custom (vertico-preselect 'directory)
  :config
  (vertico-mode))
...
(use-package vertico-multiform
  :ensure nil
  :after vertico
  :functions (vertico-multiform--toggle vertico-multiform--lookup vertico-multiform--display-menu)
  :custom
  (vertico-multiform-commands '((magit-clone (vertico-preselect . prompt))))
  :config
  (advice-add 'vertico-multiform--setup :override (lambda ()
                                                    (let ((cat (compat-call completion-metadata-get
                                                      ...
                                                      (dolist (x (cdr (or (vertico-multiform--lookup current-minibuffer-command vertico-multiform-commands)
                                                          ...
                                                          ((or (pred functionp) (pred symbolp))
                                                           (let* ((sym (and (symbolp x) (intern-soft (format "vertico-%s-mode" x))))
                                                             ...
                                                             (when (or (not (symbolp mode)) (not (boundp mode)) (not (symbol-value mode)))
                                                               (push mode modes))))
                                                          (`(,k . ,v) (progn (message "%s" k) (set (make-local-variable k) v)))
...
(use-package consult
  ...
  :hook (completion-list-mode . consult--default-completion-list-preview-setup)
...
#+BEGIN_SRC emacs-lisp
(use-package magit
  :functions (magit-commit-diff magit-toplevel magit-section--maybe-enable-long-lines-shortcuts)
  :preface
  (defun magit-dired-other-window ()
    (interactive)
    (dired-other-window (magit-toplevel)))
  (defun my-magit-section--maybe-enable-long-lines-shortcuts ()
  ...
  :bind (("C-x g" . magit-status)
         ...
         ("C-o" . magit-show-commit))
  :init
  (remove-hook 'server-switch-hook #'magit-commit-diff)
  (remove-hook 'with-editor-filter-visit-hook #'magit-commit-diff)
  :config
  (advice-add #'magit-section--maybe-enable-long-lines-shortcuts :override #'my-magit-section--maybe-enable-long-lines-shortcuts)

  ;; don’t add git-commit--dabbrev-capf since it’s broken with completion-preview-mode
  (advice-add #'git-commit-setup-capf :override (lambda ())))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package multiple-cursors
    :defines (mc--default-cmds-to-run-once)
    :functions (mc/create-fake-cursor-at-point mc/maybe-multiple-cursors-mode symbol-overlay-get-list)
    :bind
...
#+BEGIN_SRC emacs-lisp
  (use-package apropospriate-theme
    ...
    (defun apropospriate-change-theme-appearance (&optional appearance)
      ...
      (pcase appearance
        ('light (enable-theme 'apropospriate-light))
        ('dark (enable-theme 'apropospriate-dark))))
    (deftheme apropospriate-dark "The dark variant of the Apropospriate color theme")
    (deftheme apropospriate-light "The light variant of the Apropospriate color theme")
    ...
    (create-apropospriate-theme 'light 'apropospriate-light)
    (provide-theme 'apropospriate-light)
...
(use-package vterm
  ...
  (defun vterm-bell-detector (orig-fun process input)
    (when (and (string-match-p "\a" input)
               ;; Ignore bells in OSC sequences (terminal title updates)
               ;; (not (string-match-p "]0;.*\007" input))
               (not (string-match-p "\e][^\a]+\a" input))
               (fboundp 'mode-line-notify-send))
      ;; (when (string-match-p "]0;.*\007" input) (message input))
      ;; (require 'mode-line-notify)
      (let ((buffer (process-buffer process)))
        (mode-line-notify-send
         :title (format "%s Bell" (buffer-name buffer))
         :buffer buffer)))
  ...
  (advice-add 'vterm--filter :around #'vterm-bell-detector))

  (use-package vterm-toggle
    :defines (vterm-toggle-togglable-buffer-functions)
    :bind (:package vterm
                    :map vterm-mode-map
...
#+BEGIN_SRC emacs-lisp
  (use-package comint
    ...
    (defun turn-on-comint-history (file)
      (interactive)
    ...
    (defun comint-bell-detector (input)
      (when (and (string-match-p "\a" input)
                 ;; Ignore bells in OSC sequences (terminal title updates)
                 ;; (not (string-match-p "]0;.*\007" input))
                 (not (string-match-p "\e][^\a]+\a" input))
...
#+BEGIN_SRC emacs-lisp
(use-package eshell
  :defines (eshell-history-file-name)
  :custom
  ...
  :hook (;; (eshell-first-time-mode . eshell-read-history)
         ...
         (kill-emacs . emacs-solo/eshell--save-merged-history)
         (eshell-mode . (lambda () (eshell-read-history))))
  :preface
  ...
  (autoload #'eshell-read-history "eshell-read-history")
...
(use-package files
  ...
  (advice-add #'find-file
              :around (defun find-file--line-number (orig-fun filename
                                                              &optional wildcards)
...
#+BEGIN_SRC emacs-lisp
  (use-package flyspell
    ...
    :if (locate-file
         ...
         exec-path)
    :hook (((org-mode text-mode git-commit-mode) . flyspell-mode)
...
#+BEGIN_SRC emacs-lisp
(use-package proced
  :functions (proced-toggle-auto-update)
  :custom
...
#+BEGIN_SRC emacs-lisp
  (use-package project
    ...
    :config
    (advice-add #'project-or-external-find-regexp
                :before (lambda (regexp) (when (length< regexp 2) (error "search term too short"))))
...
#+BEGIN_SRC emacs-lisp
  (use-package repeat
    :demand
    :bind (:repeat-map frame-repeat-map
           ("o" . other-frame)
           ("n" . make-frame)
           ("d" . delete-frame))
    :config
...
(use-package shell
  ...
  :hook ((shell-mode . dirtrack-mode)
         ...
         (shell-mode . (lambda () (add-to-list 'comint-output-filter-functions 'comint-bell-detector)))
         )
  :config
  (advice-add #'shell
              :around (defun shell-set-remote-shell-path-maybe (orig-fun &rest args)
...
#+BEGIN_SRC emacs-lisp
  (use-package tab-bar
    ...
    :bind (:repeat-map tab-bar-history-repeat-map
...
(use-package timeclock
  :mode ("timelog\\'" . timeclock-mode)
  :functions (timeclock-generate-report timeclock-update-mode-line)
  :preface
...
(use-package whitespace
  ...
  :hook (prog-mode . (lambda () (and (not noninteractive)
...
(use-package window
  :ensure nil
  :bind (("C-x 2" . split-window-below-last-buffer)
         ("C-x 3" . split-window-right-last-buffer))
  :config
  (advice-add #'other-window :around
              (defun other-window-split-if-single (orig-fun &rest args)
                "Split the frame if there is a single window.

Use CURRENT-PREFIX-ARG to show an identical buffer."
                (if (one-window-p)
                    (when-let ((window (split-window-sensibly)))
                      (select-window window)
                      (unless current-prefix-arg
                        (let ((switch-to-prev-buffer-skip (unless prefix-arg 'this))) (switch-to-next-buffer))))
                  (apply orig-fun args))))
  ...
  (defun split-window-below-last-buffer ()
    "Vertically splitting the screen and open the previous buffer instead of
identical buffers.
...
Use CURRENT-PREFIX-ARG to show an identical buffer."
    ...
    (unless current-prefix-arg
      (let ((switch-to-prev-buffer-skip 'this)) (switch-to-next-buffer))))

  (defun split-window-right-last-buffer ()
    "Horizontally splitting the screen and open the previous buffer instead
of identical buffers.
...
(use-package xref
  ...
  (defun xref-goto-xref-same-window (&optional quit)
    (interactive "P")
    (same-window-prefix)
    (xref-goto-xref quit))
  :bind (
         :map xref--xref-buffer-mode-map
         ("E" . emacs-solo/xref-to-grep-compilation)
         ("RET" . xref-goto-xref-same-window)
         )
  :custom
...
#+BEGIN_SRC emacs-lisp
  (use-package eglot
    ...
    (add-to-list 'eglot-server-programs
                 `((haskell-mode haskell-ts-mode) . ,(eglot-alternatives
                                                      '("static-ls" ("haskell-language-server-wrapper" "--lsp")))))
    :bind (:map
           eglot-mode-map
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package ghci-compilation
    ...
    :preface
    (defun ghci-compilation-main () (interactive) (ghci-compilation--send-string "main"))
    :bind (("C-x p ." . ghci-compilation)
           ("C-x C-1" . ghci-compilation-transient-menu)
           :map haskell-mode-map
           ("C-c C-r" . ghci-compilation-reload)
           ("C-c C-l" . ghci-compilation-add-file)
           ("C-c C-m" . ghci-compilation-main)
...
(use-package claude-code-ide
  ...
  :custom (claude-code-ide-enable-mcp-server t))
#+END_SRC
...
(use-package emacs-solo-olivetti
  ...
  :hook
  (org-mode . emacs-solo/center-document-mode)
  (gnus-group-mode . emacs-solo/center-document-mode)
  (gnus-summary-mode . emacs-solo/center-document-mode)
  (gnus-article-mode . emacs-solo/center-document-mode)
  (gnus-server-mode . emacs-solo/center-document-mode)
  (gnus-browse-mode . emacs-solo/center-document-mode)
  )
#+END_SRC

**** envrc

[[https://github.com/purcell/envrc][GitHub]]
...
#+BEGIN_SRC emacs-lisp
  (use-package activities
    ...
    :preface
    (defun activities-switch-to-buffer ()
      (interactive)
    ...
    (("C-x C-a C-n" . activities-new)
     ...
     ([remap switch-to-buffer] . activities-switch-to-buffer)
     ;; ([remap switch-to-buffer-other-window] . activities-switch-to-buffer-other-window)
     ;; ([remap switch-to-buffer-other-frame] . activities-switch-to-buffer-other-frame)
     ))
#+END_SRC
...
(use-package agent-shell
  ...
  :functions (ghci-compilation--send-string ghci-compilation--find-most-likely-buffer ghci-compilation-get-last-output--marker ghci-compilation-reload-nix claude-code-ide-make-tool)
                                ...
                                (agent-shell-subscribe-to
                                 ...
                                 :on-event `(lambda (_event)
                                              (when (buffer-live-p ,buffer)
                                                (mode-line-notify-send
                                                 :title (format "%s Agent Shell Turn Complete" (buffer-name ,buffer))
                                                 :buffer ,buffer))))
                                (agent-shell-subscribe-to
                                 :shell-buffer buffer
                                 ...
                                 :on-event `(lambda (_event)
                                              (when (buffer-live-p ,buffer)
                                                (mode-line-notify-send
                                                 :title (format "%s Agent Shell Permission Request" (buffer-name ,buffer))
                                                 :buffer ,buffer)))))))
  :custom
       ...
       (let* ((project-dir (agent-shell-cwd))
         ...
         `((name . "emacs")
           ...
           (url . ,(format "http://localhost:%d/mcp/%s"
                           (claude-code-ide-mcp-server-ensure-server) session-id)))))))
  :preface
  (defun claude-ghci-compilation-reload ()
    (claude-code-ide-mcp-server-with-session-context nil
      (ghci-compilation--send-string ":r" nil t)))
  (defun claude-ghci-compilation-add (modules)
    (claude-code-ide-mcp-server-with-session-context nil
      (ghci-compilation--send-string (format ":add %s" (string-join modules " ")) nil t)))
  (defun claude-ghci-compilation-run-tests (args)
    (claude-code-ide-mcp-server-with-session-context nil
      (ghci-compilation--send-string (format ":add TestMain\n:set args %s\ntestMain" args) nil t)))
  (defun claude-ghci-compilation-set-env (name value)
    (claude-code-ide-mcp-server-with-session-context nil
      (ghci-compilation--send-string (format ":env %s %s" name value) nil t)))
  (defun claude-ghci-compilation-run-ghci-script (script)
    (claude-code-ide-mcp-server-with-session-context nil
      (ghci-compilation--send-string (format ":script %s" script) nil t)))
  (defun claude-ghci-compilation-run-test-suite (args)
    (claude-code-ide-mcp-server-with-session-context nil
      (ghci-compilation--send-string (format ":add TestMain\n:set args %s\nmain" args) nil t)))
  (defun claude-ghci-compilation-ensure-buffer ()
    (claude-code-ide-mcp-server-with-session-context nil (ghci-compilation)))
  (defun claude-ghci-compilation-reload-nix ()
    (claude-code-ide-mcp-server-with-session-context nil (ghci-compilation-reload-nix nil t)))
  ;; (defun claude-ghci-compilation-send-command (command)
  ;;   (claude-code-ide-mcp-server-with-session-context nil
  ;;     (progn (ghci-compilation--send-string command))))
  ;; (claude-code-ide-make-tool
  ;;  :function #'claude-ghci-compilation-send-command
  ;;  :name "ghci-compilation-send-command"
  ;;  :description "Send a GHCI command to the running process."
  ;;  :args '((:name "command"
  ;;                 :type string
  ;;                 :description "The command to run in GHCI.")))
  (defun claude-ghci-compilation-check-output ()
    (claude-code-ide-mcp-server-with-session-context nil
      (let* ((res (ghci-compilation-get-last-output--marker))
             (buffer (nth 0 res))
             (beg (nth 1 res))
             (end (nth 2 res)))
        (with-current-buffer buffer
          (save-excursion
            (goto-char beg)
            (while (re-search-forward "\\[[0-9]+ of [0-9]+\\] .*$" end t))
            (buffer-substring-no-properties (point) end))))))

  (defun claude-ghci-compilation-rerun-last-command ()
    (claude-code-ide-mcp-server-with-session-context nil
      (ghci-compilation--send-string (with-current-buffer (ghci-compilation--find-most-likely-buffer) (comint-previous-input-string 0)) t)))

  (defun agent-shell-new-shell-choose-agent ()
...
Always prompts for agent selection, even if existing shells are available."
  ...
  (transient-append-suffix 'agent-shell-help-menu "N"
    '("w" "New shell in git worktree" agent-shell-new-worktree-shell))

  (transient-append-suffix 'agent-shell-help-menu "C-n"
    '("C-n" "New shell (choose agent)" agent-shell-new-shell-choose-agent))

  ;; (require 'claude-code-ide-emacs-tools)
  ;; (require 'ghci-compilation)
  (claude-code-ide-make-tool
   :function #'claude-ghci-compilation-reload
   :name "ghci-compilation-reload"
   :description "Compile Haskell code to test that changes work. Perfect after making some change to a Haskell file."
   :args nil)
  (claude-code-ide-make-tool
   :function #'claude-ghci-compilation-add
   :name "ghci-compilation-add"
   :description "Add Haskell modules to scope. Run after first making changes to any module."
   :args '((:name "modules"
                  :type array
                  :description "The module names to add to scope"
                  :items (:name "module"
                                :type string
                                :description "Module name"))))
  (claude-code-ide-make-tool
   :function #'claude-ghci-compilation-run-tests
   :name "ghci-compilation-run-tests"
   :description "Run test suite"
   :args '((:name "args"
                  :type string
                  :description "Arguments to pass to test suite. Could be something like --match "/My.Module.Name/test group/test name/" --seed 1149949735.")))

  (claude-code-ide-make-tool
   :function #'claude-ghci-compilation-set-env
   :name "ghci-compilation-set-env"
   :description "Set env"
   :args '((:name "name"
                  :type string
  ...
  (claude-code-ide-make-tool
   :function #'claude-ghci-compilation-run-ghci-script
   :name "ghci-compilation-run-ghci-script"
   :description "Run ghci script"
   :args '((:name "filename"
                  :type string
                  :description "File name of script")))
  (claude-code-ide-make-tool
   :function #'claude-ghci-compilation-run-test-suite
   :name "ghci-compilation-run-test-suite"
   :description "Run test suite"
   :args '((:name "args"
                  :type string
                  :description "Args to pass")))
  (claude-code-ide-make-tool
   :function #'claude-ghci-compilation-ensure-buffer
   :name "ghci-compilation-ensure-buffer"
   :description "Ensure ghci buffer exists and is running"
   :args nil)

  (claude-code-ide-make-tool
   :function #'claude-ghci-compilation-reload-nix
   :name "ghci-compilation-reload-nix"
   :description "Reload nix develop shell"
   :args nil)

  (claude-code-ide-make-tool
   :function #'claude-ghci-compilation-check-output
   :name "ghci-compilation-check-output"
   :description "Get last output from GHCI."
   :args nil)

  (claude-code-ide-make-tool
   :function #'claude-ghci-compilation-rerun-last-command
   :name "ghci-compilation-rerun-last-command"
   :description "Rerun last command from GHCI."
   :args nil)
  )

  (use-package chatgpt-shell
    :bind (("C-c C-g" . chatgpt-shell-transient)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package copilot
    :hook
    (((text-mode prog-mode) . copilot-mode)
...
#+BEGIN_SRC emacs-lisp
  (use-package dape
    :disabled
    :hook (prog-mode . dape-breakpoint-mode))
#+END_SRC
...
(use-package dumb-jump
  :functions (dumb-jump-get-language-from-aliases)
  :autoload (dumb-jump-xref-activate)
  :init
  (add-hook 'xref-backend-functions #'dumb-jump-xref-activate 90)
  :config
  (advice-add #'dumb-jump-get-language-from-aliases
              :override (lambda (lang)
                          ...
                          (assoc-default lang '(("sh" . "shell") ("shell" . "shell") ("cperl" . "perl")
                                                ...
                                                ("R" . "r") ("r" . "r") ("ghci-compilation" . "haskell"))))))
#+END_SRC
...
(use-package dwim-shell-command
  :functions (dwim-shell-command-on-marked-files)
...
#+BEGIN_SRC emacs-lisp
  (use-package mode-line-notify
    :ensure nil
    :hook (after-init . mode-line-notify))
#+END_SRC
...
#+BEGIN_SRC shell
    export PATH="$HOME/bin":"$HOME/.nix-profile/bin":$PREFIX/bin:"$HOME/.local/share/node_modules/bin":"$HOME/.local/bin":@PATH@${PATH:+:$PATH} \
...
** =.zshrc=
:PROPERTIES:
:header-args: :tangle zshrc.sh :tangle-mode (identity #o755) :shebang "#!/usr/bin/env zsh"
:CUSTOM_ID: zshrc
:END:
...
#+BEGIN_SRC shell
  ...
  # Set to "real" to prevent other zsh instances from deleting useful history
  HISTFILE="$HOME/.zsh_real_history"
  REPORTTIME=4
  DIRSTACKSIZE=16
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; (setopt grep-find-command `(,(concat ripgrep-executable " -n --no-heading -e '' $(git rev-parse --show-toplevel || pwd)") . 86))
...
(Nixpkgs-unstable has version 30.2 currently).

#+BEGIN_SRC nix
  customEmacsPackages = let
    # I’ve switched over the years between macport and non-macport. latest non-macport seems pretty good recently.
    # emacsBinary = if hostPlatform.isDarwin then pkgs'.emacs-macport else pkgs'.emacs;
    emacsBinary = pkgs'.emacs-unstable;
    pkgs' = import pkgs.path {
      inherit crossSystem system config;
      overlays = overlays ++ [ emacs-overlay.overlay ];
    };
    # use native tuning for better performance
    customEmacsPackages' = pkgs'.emacsPackagesFor (emacsBinary.overrideAttrs (o: { env = o.env // { NIX_CFLAGS_COMPILE = (o.env.NIX_CFLAGS_COMPILE or "") + "-Os -march=native -mtune=native"; }; }));
  in # override elpaPackages so elpaDevelPackages is picked up
     customEmacsPackages'.overrideScope (eself: esuper: esuper.override { elpaPackages = {}; });
#+END_SRC

Tex live provides some LaTeX commands for us.
...
#+BEGIN_SRC nix
    ...
    install -D ${ensure ./site-lisp/claude-code-ide.el
                         "site-lisp/claude-code-ide.el"} \
            $out/share/emacs/site-lisp/claude-code-ide.el
    install -D ${ensure ./site-lisp/claude-code-ide-debug.el
                         "site-lisp/claude-code-ide-debug.el"} \
            $out/share/emacs/site-lisp/claude-code-ide-debug.el
    install -D ${ensure ./site-lisp/claude-code-ide-diagnostics.el
                         "site-lisp/claude-code-ide-diagnostics.el"} \
            $out/share/emacs/site-lisp/claude-code-ide-diagnostics.el
    install -D ${ensure ./site-lisp/claude-code-ide-emacs-tools.el
                         "site-lisp/claude-code-ide-emacs-tools.el"} \
            $out/share/emacs/site-lisp/claude-code-ide-emacs-tools.el
    install -D ${ensure ./site-lisp/claude-code-ide-mcp.el
                         "site-lisp/claude-code-ide-mcp"} \
            $out/share/emacs/site-lisp/claude-code-ide-mcp.el
    install -D ${ensure ./site-lisp/claude-code-ide-mcp-handlers.el
                         "site-lisp/claude-code-ide-mcp-handlers.el"} \
            $out/share/emacs/site-lisp/claude-code-ide-mcp-handlers.el
    install -D ${ensure ./site-lisp/claude-code-ide-mcp-http-server.el
                         "site-lisp/claude-code-ide-mcp-http-server.el"} \
            $out/share/emacs/site-lisp/claude-code-ide-mcp-http-server.el
    install -D ${ensure ./site-lisp/claude-code-ide-mcp-server.el
                         "site-lisp/claude-code-ide-mcp-server.el"} \
            $out/share/emacs/site-lisp/claude-code-ide-mcp-server.el
    install -D ${ensure ./site-lisp/claude-code-ide-transient.el
                         "site-lisp/claude-code-ide-transient.el"} \
            $out/share/emacs/site-lisp/claude-code-ide-transient.el
    install -D ${ensure ./site-lisp/mode-line-notify.el
                         "site-lisp/mode-line-notify.el"} \
            $out/share/emacs/site-lisp/mode-line-notify.el
    install -D ${ensure ./site-lisp/apropospriate-theme.el
                         "site-lisp/apropospriate-theme.el"} \
            $out/share/emacs/site-lisp/apropospriate-theme.el
  '';
#+END_SRC
...
Setup the system headers path. This is deliberately light to avoid huge
...
include directory. This is only useful on macOS/Darwin machines.
...
This includes some extra files that are checked in, as well as some tips. They
mostly deal with maintainence and advanced usage of BAUER.

** Helpful tips

On macOS, you may want to disable some shortcuts that override the Emacs ones. I
believe this will do that for M-s-d
...
Then add it to macOS keychain with:

#+BEGIN_SRC sh
ssh-add --apple-use-keychain
#+END_SRC

Note: only ssh keys with passphrases can be added to the keychain. You can
temporarily add a password with `ssh-keygen -p -f`.
...
*** Mono-repos performance

There are a few useful things to help manage large mono-repos. Unfortunately,
they need to be enabled per-repo:
...
You might like these packages that I use a lot. They aren’t a requirement to use
this environment though:
...
nix profile install nixpkgs#pwgen
nix profile install nixpkgs#hunspell
nix profile install nixpkgs#hunspellDicts.en_US
...
Then, add =$HOME/.ssh/id_ed25519=.pub through GitHub’s Web UI. Invoke the script
with:
