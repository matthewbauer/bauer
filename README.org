...
#+BEGIN_SRC nix :padline no
  ...
  { pkgs ? import <nixpkgs> {}
  , ensure ? (f: n: f)
  , isSmall ? (pkgs.config.bauer.small or true) }:
  with pkgs; let isBig = !isSmall;
#+END_SRC
...
#+BEGIN_SRC nix
    site-paths = runCommand "site-paths.el" ({
      ...
      inherit ripgrep ag fortune gnutls nethack
              ...
              git bash zsh diffutils man gawk
...
#+BEGIN_SRC nix
  userPackages = [
    ...
    # GUI Apps
    # transmission-gtk inkscape gimp
    # retroarch
    # blender chromium wesnoth kodi
    # firefox vlc virtualbox
...
in buildEnv {
  ...
  passthru = with lib; {
    ...
    INFOPATH = makeSearchPath "share/info" [info-pages];
    XDG_DATA_DIRS = makeSearchPath "share" [xdg-data-dirs];
  };
  paths = [
    myEmacs zsh
