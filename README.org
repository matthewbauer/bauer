    ...
    (when (and (string-match-p "\a" input)
      ...
      (let ((buffer (current-buffer)))
        (mode-line-notify-send
         ...
         :action `(lambda () (switch-to-buffer ,buffer)))))
...
(use-package claude-code-ide
  ...
  (defun claude-ghci-compilation-add (modules)
    (claude-code-ide-mcp-server-with-session-context nil
      (ghci-compilation--send-string (format ":add %s" modules))))
  (claude-code-ide-make-tool
  ...
  (claude-code-ide-make-tool
   ...
   :args '((:name "match"
                  :type string
                  :description "A pattern matching any tests to run. Separate multiple matches by | (pipe)")))

  (defun claude-ghci-compilation-run-ghci (script)
    (claude-code-ide-mcp-server-with-session-context nil
      (ghci-compilation--send-string (format ":script %s" script))))
  (claude-code-ide-make-tool
   :function #'claude-ghci-compilation-run-tests
...
#+BEGIN_SRC emacs-lisp
  (use-package agent-shell
    :config (setq agent-shell-preferred-agent-config (agent-shell-anthropic-make-claude-code-config))
