...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package tramp-term
    ...
    :hook (tramp-term-after-initialized . (lambda (_)
...
#+BEGIN_SRC nix :padline no
  ...
  , ... }: let
  inherit (pkgs) stdenv lib runCommand runCommandCC buildEnv config writeText;
...
#+BEGIN_SRC nix
  gccemacs = (pkgs.emacs.override { stdenv = pkgs.gcc9Stdenv; srcRepo = true; }).overrideAttrs (o: {
    src = pkgs.fetchgit {
    ...
    patches = [];
    buildInputs = o.buildInputs or [] ++ [ pkgs.libgccjit pkgs.jansson ];
    configureFlags = o.configureFlags or [] ++ ["--with-nativecomp"];
  });

  emacs-overlays = pkgs.fetchFromGitHub {
    owner = "nix-community";
    repo = "emacs-overlay";
    rev = "8c0a391ff034e54e57a7525fbb4074df9ce3d338";
    sha256 = "sha256-MXA6+y70GPz/hu5+3uDjgJbWhAF40fh+S2WO1tSYEBg=";
  };

  customEmacsPackages = (import emacs-overlays pkgs pkgs).emacsPackagesFor
    (if hostPlatform.isDarwin then pkgs.emacsMacport
     else if hostPlatform.isLinux then gccemacs
     else pkgs.emacs);
#+END_SRC
...
#+BEGIN_SRC nix
  default = runCommandCC "bauer-emacs" {
    nativeBuildInputs = [ customEmacsPackages.emacs pkgs.git ];
    LIBRARY_PATH = "${lib.getLib stdenv.cc.libc}/lib";
  } ''
#+END_SRC
...
emacs --batch --quick \
      --eval \
"(let ((default-directory \"${myEmacsPackages}/share/emacs/site-lisp\"))
  (normal-top-level-add-subdirs-to-load-path))" \
      -L . -f batch-native-compile *.el
...
#+BEGIN_SRC nix
  emacsWrapper = explicitRequires:
   runCommandCC "emacs-packages-deps"
   { nativeBuildInputs = [ pkgs.xorg.lndir customEmacsPackages.emacs pkgs.git ];
...
#+BEGIN_SRC nix
  myEmacsPackages = emacsWrapper
    ((requiredPackages customEmacsPackages myEmacsPackages')
     ++ (with customEmacsPackages; [use-package delight]));

  myEmacs = customEmacsPackages.emacsWithPackages (epkgs: [myEmacsPackages default]);
#+END_SRC
