...
#+BEGIN_SRC shell
  ...
  if [ -n "${BASH_VERSION-}" ]; then
    ...
    if [ -n "${PS1-}" ] && [ "${TERM-}" != dumb ] && shopt -q progcomp; then
...
#+BEGIN_SRC shell
alias rm='rm -iv -I --preserve-root' \
      mv='mv -iv' \
      cp='cp -iv' \
      ln='ln -iv' \
      rmdir='rmdir -v' \
  ...
  if [ -n "${SSH_CONNECTION-}" ]; then
      ...
      # private network
          ...
          if command -v avahi-resolve-address > /dev/null; then
              hostname="$(avahi-resolve-address "$server_ip" 2>/dev/null | awk '{ print $2 }')"
          fi
...
if [ "${TERM_PROGRAM-}" = iTerm.app ]; then
  ...
  iterm2_print_state_data() {
    printf "\e]1337;RemoteHost=%s@%s\007" "${USER-$LOGNAME}" "${HOST-}"
    printf "\e]1337;CurrentDir=%s\007" "$PWD"
  }
  iterm2_after_cmd_executes() {
    printf "\e]133;D;%s\007" "$?"
    iterm2_print_state_data
  }
...
#+BEGIN_SRC nix :padline no
  ...
  , emacs-overlay ? { overlay = import (pkgs.fetchFromGitHub {
      owner = "nix-community";
      repo = "emacs-overlay";
      rev = "b9e017b0d91c18f404f3e62cce1329b34d75778c";
      sha256 = "1kih2x5ji4985yqkz1vnl34y5aq5q93061z80b9wjj9ha3hsdc0f";
    }); }
  , ... }: let
