...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package readline-complete
    ...
    :hook (shell-mode . (lambda () (setq-local company-backends '(company-readline)))))
  (use-package company-irony
    :commands company-irony
    :hook (irony-mode . (lambda () (setq-local company-backends '(company-irony)))))
  (use-package company-restclient
    :commands company-restclient
    :hook (restclient-mode . (lambda () (setq-local company-backends '(company-restclient)))))
  (use-package company-anaconda
    :commands company-anaconda
    :hook (anaconda-mode . (lambda () (setq-local company-backends '(company-anaconda :with company-capf)))))
  (use-package company-jedi
    :commands company-jedi
    :hook (python-mode . (lambda () (setq-local company-backends '(company-jedi)))))
  (use-package company-tern
    :commands company-tern
    :hook (tern-mode . (lambda () (setq-local company-backends '(company-tern)))))
  (use-package company-ghc
    :commands company-ghc
    :hook (haskell-mode . (lambda () (setq-local company-backends '(company-ghc)))))
...
#+BEGIN_SRC emacs-lisp
  (use-package magithub
    :disabled
    :commands (magithub-browse magithub-create magithub-fork magithub-clone)
    :preface
    (autoload 'magithub-dispatch-popup "magithub" nil t)
    :bind (:package magit
                    :map magit-commit-section-map
                    ("w" . magithub-commit-browse)
    ...
    :hook ((magit-mode . (lambda ()
           ...
           (magit-status-headers . magithub-maybe-insert-ci-status-header)
           (git-commit-setup . magithub-edit-mode-hook)
           (magithub-edit-mode . magithub-edit-mode-hook)
           (magit-status-sections . magithub-issue--insert-pr-section)
           (magit-status-sections . magithub-issue--insert-issue-section)
           ))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package term
    ...
    (defun my-term (&optional path name)
      (interactive)
      (set-buffer (make-term "my-term" zsh-command))
      (term-mode)
      (term-char-mode)
      (term-set-escape-char ?\C-x)
...
#+BEGIN_SRC nix
    ...
    site-paths = runCommand "site-paths.el" {
      inherit rtags ripgrep ag ant nethack fortune gnutls
        ...
        gdb coq
        manpages sysheaders sysframeworks schemas infoPages
        # mariadb go golint
      ;
...
#+BEGIN_SRC nix
  infoPages = buildEnv {
    name = "info-pages";
    paths = packages ++ [customEmacsPackages.emacs];
  ...
  manpages = buildEnv {
    name = "man-pages";
    paths = packages ++ [posix_man_pages
                         customEmacsPackages.emacs] # stdmanpages
...
#+BEGIN_SRC nix
  # pkgs.proofgeneral = proofgeneral.override {enableDoc = true;};
  ...
  if [ -w $out/share/mime ] && [ -w $out/share/mime/packages ]; then
    XDG_DATA_DIRS=$out/share ${shared-mime-info}/bin/update-mime-database \
      -V $out/share/mime
  fi
