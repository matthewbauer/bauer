   ...
   '(display-buffer-alist
     '(("^\\*Ilist\\*$" imenu-list-display-buffer)
       ...
       ("\\\\*vterm\\\\*"
        ...
        (dedicated . t))
       ("\\\\*Occur\\\\*\\\\|\\\\*grep\\\\*\\\\|\\\\*xref\\\\*\\\\|\\\\*ripgrep-search\\\\*" display-buffer-reuse-window
        (inhibit-same-window))
       ...
       ("\\\\*e?shell\\\\*" display-buffer-in-direction
        ...
        (window . root)
        (window-height . 0.3))
       ("\\\\*grep\\\\*\\\\|\\\\*compilation\\\\*\\\\|\\\\*ripgrep-search\\\\*" display-buffer-in-side-window
        (side . right)
        (slot . 0)
        (window-parameters
...
#+BEGIN_SRC emacs-lisp
  (use-package company
    ...
    :custom (company-auto-complete (lambda ()
                              (and (company-tooltip-visible-p)
                                   (company-explicit-action-p))))
    :custom (company-continue-commands
      '(not save-buffer
            save-some-buffers
