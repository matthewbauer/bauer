...
integration between Emacs and Nix comes with lots of useful side effects.
...
** Increase GC
   ...
   startup. After that we reset it to the standard value.
...
** Helpers
   ...
   Right now hook-helpers is all that’s needed. It provides some shortcuts to
   defining functions and adding hooks.
...
substituteInPlace $out/etc/profile.d/my-profile.sh \
  ...
  --replace @cacert@ ${cacert}
    ...
    ++ [stack ghc]
    ++ (with haskellPackages; [cabal-install]); #ghc-mod
};
   #+END_SRC
