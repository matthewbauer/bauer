...
(use-package xterm-color
  ...
  :hook (eshell-before-prompt . (lambda () (setq-local xterm-color-preserve-properties t))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package url-util
    :ensure nil
    :functions (url-hexify-string url-unhex-string)
    :preface
...
#+BEGIN_SRC shell :padline no
  export PATH=$HOME/.nix-profile/bin:$PREFIX/bin:@PATH@
...
Setup Apple Terminal so that CWD is shown and sessions are saved.
...
shell_session_delete_expired() {
    if [ ! -e "$SHELL_SESSION_TIMESTAMP_FILE" ] || \
       [ -z "$(find "$SHELL_SESSION_TIMESTAMP_FILE" -mtime -1)" ]; then
        ...
        if shlock -f "$expiration_lock_file" -p $$; then
            ...
            local delete_count=$(find "$SHELL_SESSION_DIR" -type f -mtime +14 -print -delete | wc -l)
            if [ "$delete_count" -gt 0 ]; then
                echo $delete_count' completed.'
            else
                echo 'none found.'
            fi
   ...
   && [ -n "$TERM_SESSION_ID" ]; then
    ...
    if [ ${SHELL_SESSION_HISTORY:-1} -eq 1 ]; then
        ...
        if [ -s "$SHELL_SESSION_HISTFILE" ]; then
            ...
            precmd_functions+=(shell_session_history_check)
        fi
    fi
    SHELL_SESSION_TIMESTAMP_FILE="$SHELL_SESSION_DIR/_expiration_check_timestamp"
    exit_trap() { shell_session_save && shell_session_delete_expired }
    trap exit_trap EXIT
fi
#+END_SRC
...
#+BEGIN_SRC nix
  myEmacsPackages = buildEnv {
    ...
    paths = (requiredPackages pkgs.emacsPackagesNg myEmacsPackages')
          ++ [ pkgs.emacsPackagesNg.use-package pkgs.emacsPackagesNg.delight ];
  };
...
#+END_SRC
** LICENSE
   Copyright (C) 2018-2019 Matthew Bauer
