...
(use-package minibuffer
  :ensure nil
  :functions (keyboard-quit@quit-current-context)
  :custom
...
(use-package emacs-solo-dired-gutter
  :ensure nil
  :no-require t
  :functions (vc-git-root vc-git--run-command-string dired-get-filename)
  :preface
  ...
  (defun emacs-solo/dired-git-status-overlay ()
    ...
    (let ((git-root (ignore-errors (vc-git-root default-directory))))
      (when (and git-root
        ...
        (let* ((git-status (vc-git--run-command-string nil "status" "--porcelain" ;; "--ignored"
  ...
  :hook (dired-after-readin . emacs-solo/dired-git-status-overlay))
#+END_SRC
...
Called by `window-selection-change-functions' on C-x o, etc."
  ...
  :bind
  ("C-c g p" . emacs-solo/goto-previous-hunk)
  ("C-c g r" . emacs-solo/git-gutter-off)
  ("C-c g g" . emacs-solo/git-gutter-on)
  ("C-c g n" . emacs-solo/goto-next-hunk)

  :hook (after-init . emacs-solo/git-gutter-on))
#+END_SRC
...
#+BEGIN_SRC nix
  ...
  in # override elpaPacakges so elpaDevelPackages is picked up
