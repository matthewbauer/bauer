...
#+BEGIN_SRC emacs-lisp
  (set-defaults
   ...
   '(dante-methods '(styx new-impure-nix stack mafia bare-cabal bare-ghci))
   '(dante-repl-command-line-methods-alist
     '((styx  . ,(lambda (root) (dante-repl-by-file root '("styx.yaml") '("styx" "repl" dante-target))))
       (nix   . ,(lambda (root) (dante-repl-by-file root '("shell.nix" "default.nix") '("nix-shell" "--run" (concat "cabal new-repl " (or dante-target "") " --builddir=dist/dante")))))
       ...
       (new-build . ,(lambda (root) (when (or (directory-files root nil ".+\\.cabal$") (file-exists-p "cabal.project")) '("cabal" "new-repl" dante-target "--builddir=dist/dante"))))
